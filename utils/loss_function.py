...
def Cross_entropy_plus_Dice_loss(pred, target, alpha=0.5):
    # Calculate Cross-Entropy Loss
    ce_loss = F.cross_entropy(pred, target)
    # Calculate Dice Loss
    pred_sigmoid = torch.sigmoid(pred)
    dice_loss_val = dice_loss(pred_sigmoid[:, 0], target.float())
    
    combined_loss = alpha * ce_loss + (1 - alpha) * dice_loss_val
